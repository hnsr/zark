...
static ZKey zTranslateXKey(KeySym keysym)
{
    switch (keysym) {
        ...
        case XK_space:      return KEY_SPACEBAR;

        case XK_ISO_Level3_Shift: return KEY_RALT;
        ...
        case XK_bracketleft:    return KEY_LSQBRACKET;
        case XK_bracketright:   return KEY_RSQBRACKET;
        ...
        case XK_Sys_Req:
        case XK_Print:          return KEY_PRTSCN;

        case XK_Scroll_Lock:    return KEY_SCRLK;
        ...
        case XK_KP_Delete:      return KEY_KP_DELETE;
...
void zProcessEvents(void)
{
    ...
    while (XPending(dpy)) {
        ...
        case ButtonRelease:
            {
                ZKeyEvent zkev;
                zkev.key = KEY_MOUSE1 + (event.xbutton.button-1);
...
char *zGetUserDir(void)
{
    ...
    if (!initialized) {
        ...
        if ( (strlen(userdir) + strlen(Z_DIR_USERDATA) + 1) < Z_MAX_PATH ) {
            ...
            zWarning("Home directory + user data directory exceeded buffer dize, using empty"
                " string.");
            userdir[0] = '\0';
        }
